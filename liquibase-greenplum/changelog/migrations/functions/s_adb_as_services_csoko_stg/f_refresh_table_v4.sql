...
--changeset codex:f_refresh_table_v4 runOnChange:true splitStatements:false
DROP FUNCTION IF EXISTS s_adb_as_services_csoko_stg.f_refresh_table_v4(text, text, text, text, text, text, bool, bool, bool);

CREATE OR REPLACE FUNCTION s_adb_as_services_csoko_stg.f_refresh_table_v4(
...
DECLARE
    ...
    v_select_cols text;
    v_target_cols text;
    v_target_select_cols text;
    v_sql         text;
    v_source_from text;
    v_source_kind text;
    v_source_filter text;
    v_filter_where_sql text := '';
    v_filter_and_sql text := '';
    v_tmp_table   text;
...
BEGIN
    /*
      ...
      The source object is always exposed to p_source_filter as alias "src".
      Examples:
        src.event_dt >= DATE '2026-04-01'
        src.amount > 100
    */
    v_source_kind := lower(coalesce(nullif(btrim(p_source_kind), ''), 'table'));
    v_source_filter := nullif(btrim(p_source_filter), '');
    ...
    IF v_source_filter IS NOT NULL AND lower(left(v_source_filter, 5)) = 'where' THEN
        v_source_filter := nullif(btrim(substr(v_source_filter, 6)), '');
    END IF;

    IF v_source_filter IS NOT NULL THEN
        IF v_source_filter ~ '(;|--|/\*|\*/)' THEN
            v_is_error := true;
            v_rows := -1;
            v_error := 'Source filter must be a single SQL predicate without statement separators or comments';
        ELSE
            v_filter_where_sql := format(' WHERE (%s)', v_source_filter);
            v_filter_and_sql := format(' AND (%s)', v_source_filter);
        END IF;
    END IF;
    ...
    IF NOT v_is_error THEN
        SELECT
            ...
            string_agg(format('src.%I', t.column_name), ', ' ORDER BY t.ordinal_position)
          INTO v_target_cols, v_target_select_cols
    ...
    IF NOT v_is_error THEN
        IF v_source_kind = 'table' THEN
            v_source_from := format('%I.%I AS src', p_schema_src, p_object_src);

            SELECT
                string_agg(quote_ident(s.column_name), ', ' ORDER BY s.ordinal_position),
                string_agg(format('src.%I', s.column_name), ', ' ORDER BY s.ordinal_position)
        ...
        ELSE
            ...
            v_select_cols := v_target_select_cols;
        ...
        v_extra := v_extra || format(
            'source_kind=%s; source_from=%s; source_filter=%s; ',
            v_source_kind,
            v_source_from,
            coalesce(v_source_filter, 'null')
        );
    END IF;
    ...
    */
    IF NOT v_is_error THEN
        ...
        IF v_has_ctl_loading AND v_source_filter IS NULL THEN
            v_sql := format(
                'SELECT COUNT(*)
                   FROM (
                  ...
                  WHERE NOT EXISTS (
                        SELECT 1
                          FROM (
                               SELECT DISTINCT src.%4$I AS ctl_loading
                                 FROM %5$I.%6$I AS src
                                WHERE src.%4$I IS NOT NULL%7$s
                          ) s
            ...
            END;
        ELSIF v_has_ctl_loading THEN
            v_extra := v_extra || format(
                'ctl_loading_check_skipped=true; reason=source_filter_is_set; source_ctl_loading_col=%s; target_ctl_loading_col=%s; ',
    ...
    */
    IF NOT v_is_error AND v_effective_truncate THEN
        BEGIN
            ...
            v_sql := format(
                'INSERT INTO %I (%s) SELECT %s FROM %s%s',
                v_tmp_table,
                v_cols,
                v_select_cols,
                v_source_from,
    ...
    IF NOT v_is_error AND v_effective_truncate THEN
        BEGIN
            ...
            v_sql := format(
                'INSERT INTO %I.%I (%s) SELECT %s FROM %I AS src',
                p_schema_target,
                p_table_target,
                v_cols,
                v_select_cols,
    ...
    IF NOT v_is_error THEN
        BEGIN
            ...
            IF p_compare_row_count AND v_effective_truncate AND v_src_row_count != v_tgt_row_count THEN
                v_is_error := true;
                ...
                END IF;
            ELSIF p_compare_row_count AND NOT v_effective_truncate THEN
                v_extra := v_extra || 'row_count_compare_skipped=true; reason=append_mode; ';
            END IF;
        EXCEPTION WHEN OTHERS THEN
            GET STACKED DIAGNOSTICS
                v_sqlstate = RETURNED_SQLSTATE,
    ...
    BEGIN
        ...
        VALUES (
            gen_random_uuid(),
            's_adb_as_services_csoko_stg.f_refresh_table_v4(text, text, text, text, text, text, bool, bool, bool)',
    ...
    IF v_is_error THEN
        RAISE INFO 'Error in f_refresh_table_v4: %', v_error;
    END IF;
